...
export type AdminDrive = {
  id: string;
  kind: "quark" | "p115" | "p123" | "pikpak" | "wopan" | "onedrive" | "googledrive" | "localstorage" | "spider91" | "ftp";
  name: string;
  rootId: string;
  status: string;
  lastError?: string;
...
/** 硬删除单个视频（清理本地文件 + 删 DB 记录） */
export function deleteVideo(id: string) {
  return request<{ ok: boolean; removedFiles: number }>(
...
/** spider91 导入请求体格式（与 spider_91porn.py 输出一致） */
export type Spider91ImportEntry = {
  title: string;
  detail_url: string;
  thumb_url: string;
  viewkey: string;
...
export type Spider91ImportResult = {
  total: number;
  new: number;
  skipped: number;
  failed: number;
...
/** 从 JSON 批量导入视频到指定 spider91 drive */
export function importSpider91Videos(driveID: string, body: Spider91ImportEntry[] | { videos: Spider91ImportEntry[] }) {
  return request<{ ok: boolean; result: Spider91ImportResult }>(
    `/spider91/${encodeURIComponent(driveID)}/import`,
    { method: "POST", body: JSON.stringify(body) }
  );
}

/** 批量删除指定 spider91 drive 下所有视频 */
export function deleteSpider91Videos(driveID: string) {
  return request<{ ok: boolean; total: number; deleted: number; skipped: number }>(
    `/spider91/${encodeURIComponent(driveID)}/videos`,
    { method: "DELETE" }
  );
}
