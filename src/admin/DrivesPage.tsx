...
export function DrivesPage() {
  ...
  const importDriveIdRef = useRef("");
  const [importJsonText, setImportJsonText] = useState("");
  const [importJsonResult, setImportJsonResult] = useState<api.Spider91ImportResult | null>(null);
  const [clearVideosTarget, setClearVideosTarget] = useState<api.AdminDrive | null>(null);
  const [clearingVideosId, setClearingVideosId] = useState("");
  ...
  async function handleImportJson(d: api.AdminDrive) {
    setImportJsonText("");
    setImportJsonResult(null);
    setImportingDriveId("");        // 打开弹窗时不设置 importingDriveId，避免按钮一上来就显示"导入中"
    setImportJsonModalOpen(true);
    importDriveIdRef.current = d.id; // 存到 ref，confirmImportJson 里用
  }

  async function confirmImportJson() {
    const driveId = importDriveIdRef.current;
    if (!driveId) return;
    ...
    try {
      ...
      show("JSON 格式无效", "error");
      return;
    }

    // 支持两种格式：完整格式 {videos: [...]} 或纯数组 [...]
    let entries: api.Spider91ImportEntry[];
    if (Array.isArray(parsed)) {
      entries = parsed as api.Spider91ImportEntry[];
    } else if (parsed && typeof parsed === "object" && "videos" in parsed && Array.isArray((parsed as Record<string, unknown>).videos)) {
      entries = (parsed as Record<string, unknown>).videos as api.Spider91ImportEntry[];
    } else {
      show("JSON 格式不正确：需要 {\"videos\":[...]} 或 [{...}]", "error");
      return;
    }
    ...
    try {
      const resp = await api.importSpider91Videos(driveId, entries);
      setImportJsonResult(resp.result);
      show(`导入完成：新增 ${resp.result.new}，跳过 ${resp.result.skipped}，失败 ${resp.result.failed}`, "success");
    } catch (e) {
  ...
  async function handleClearVideos(d: api.AdminDrive) {
    setClearVideosTarget(d);
  }

  async function confirmClearVideos() {
    if (!clearVideosTarget) return;
    setClearingVideosId(clearVideosTarget.id);
    try {
      const resp = await api.deleteSpider91Videos(clearVideosTarget.id);
      show(`已删除 ${resp.deleted} 个视频${resp.skipped > 0 ? `，跳过 ${resp.skipped} 个` : ""}`, "success");
      setClearVideosTarget(null);
      refresh();
    } catch (e) {
      show(e instanceof Error ? e.message : "删除失败", "error");
    } finally {
    ...
    return (
      <section>
          ...
          <div>
            <div className="admin-detail-card">
              ...
              <div className="admin-detail-actions">
                ...
                {d.kind === "spider91" && (
                  <>
                    <button type="button" className="admin-btn" onClick={() => handleImportJson(d)}>
                      <Plus size={13} /> 导入 JSON
                    </button>
                    <button type="button" className="admin-btn is-danger" onClick={() => handleClearVideos(d)}>
                      <Trash2 size={13} /> 清空视频
                    </button>
                  </>
                )}
        ...
        <Modal
          ...
          title={`导入视频到「${selectedDrive?.name ?? selectedDrive?.id ?? ""}」`}
          onClose={() => setImportJsonModalOpen(false)}
          footer={
            <>
              <button type="button" className="admin-btn" onClick={() => setImportJsonModalOpen(false)} disabled={!!importingDriveId}>
                取消
              </button>
              <button
                type="button"
                ...
                disabled={!!importingDriveId || importJsonText.trim().length === 0}
              >
                {importingDriveId ? "导入中..." : "开始导入"}
              </button>
            </>
          }
        >
          <div className="admin-form">
            <div className="admin-form__row">
              <label>粘贴 JSON 数据（支持 spider_91porn.py 输出格式）</label>
              <textarea
                value={importJsonText}
                onChange={(e) => setImportJsonText(e.target.value)}
                placeholder={`格式示例：\n{"keyword":"学生","videos":[{"title":"...","video_url":"https://...","thumb_url":"https://...","source_id":"1210742",...}]}`}
                rows={10}
            ...
            {importJsonResult && (
              <div className="admin-form__row" style={{ marginTop: 8 }}>
                <dl className="admin-kv">
                  <dt>总数</dt><dd>{importJsonResult.total}</dd>
                  <dt>新增</dt><dd style={{ color: "var(--success)" }}>{importJsonResult.new}</dd>
                  <dt>跳过</dt><dd style={{ color: "var(--text-faint)" }}>{importJsonResult.skipped}</dd>
                  <dt>失败</dt><dd style={{ color: importJsonResult.failed > 0 ? "var(--danger)" : "var(--text-faint)" }}>{importJsonResult.failed}</dd>
                </dl>
              </div>
            )}
          </div>
        ...
        <ConfirmModal
          open={clearVideosTarget !== null}
          title="清空 spider91 视频"
          message={`确定要删除「${clearVideosTarget?.name ?? ""}」下的所有视频吗？此操作会清理所有本地视频文件和封面，不可恢复。`}
          confirmText="确认清空"
          danger
          loading={clearingVideosId === clearVideosTarget?.id}
          onCancel={() => {
            if (!clearingVideosId) setClearVideosTarget(null);
          }}
          onConfirm={confirmClearVideos}
        />
      </section>
    );
  }
