export type Kind = "quark" | "p115" | "p123" | "pikpak" | "wopan" | "onedrive" | "googledrive" | "localstorage" | "spider91" | "ftp";
...
export function usesRootDirectoryID(kind: Kind): boolean {
  return kind !== "localstorage" && kind !== "spider91" && kind !== "ftp";
}
...
export function credentialHelp(kind: Kind, isEdit: boolean): string {
  ...
  switch (kind) {
    ...
    case "ftp":
      return "连接到 FTP 服务器，扫描远程目录中的视频文件，播放时按需下载到本地缓存。FTP 为标准被动模式（PASV），请确保服务器防火墙开放了数据端口范围。";
    default:
      return "";
  }
}
      ...
      return [
        {
          ...
          required: true,
          help: "FTP 服务器的 IP 地址或域名。",
        },
        {
          key: "port",
          label: "端口",
          placeholder: "21",
          help: "FTP 服务器端口，默认 21。",
        },
        {
          key: "username",
          label: "用户名",
        ...
        },
        {
          key: "password",
          label: "密码",
          placeholder: "FTP 密码（匿名登录留空）",
        },
        {
          key: "rootPath",
          label: "远程起始路径",
          placeholder: "/videos",
          help: "从 FTP 服务器的哪个目录开始递归扫描视频文件。留空 = 从根目录 / 开始。",
        },
      ];
  }
}
