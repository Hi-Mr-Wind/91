...
import { Edit, RefreshCw, Search, CheckSquare, Square, Image, Trash2 } from "lucide-react";
...
export function VideosPage() {
  ...
  async function handleDelete(v: api.AdminVideo) {
    setDeleteTarget(v);
  }
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      const r = await api.deleteVideo(deleteTarget.id);
      show(`已删除，清理了 ${r.removedFiles} 个本地文件`, "success");
  ...
  async function handleBatchDelete() {
    ...
    try {
      ...
      show(`批量删除完成，成功 ${success} / ${ids.length} 个`, success === ids.length ? "success" : "info");
      setSelectedIds(new Set());
    } finally {
  ...
  return (
    <section>
        ...
        <div className="admin-videos-list-toolbar">
          ...
          {selectedIds.size > 0 && (
            <div className="admin-videos-bulk-actions">
              ...
              <button type="button" className="admin-btn is-danger" onClick={handleBatchDelete} disabled={batchDeleting}>
                <Trash2 size={13} /> {batchDeleting ? "删除中..." : "批量删除"}
              </button>
            </div>
          )}
        </div>
              ...
              {listItems.map((v) => (
                <tr key={v.id} className={selectedIds.has(v.id) ? "is-selected" : ""}>
                  ...
                  <td className="is-actions" data-label="操作">
                    ...
                    <button type="button" className="admin-btn is-danger" onClick={() => handleDelete(v)} title="删除视频">
                      <Trash2 size={13} />
                    </button>
                  </td>
                </tr>
      ...
      <ConfirmModal
        ...
        title="删除视频"
        message={`确定要删除视频「${deleteTarget?.title ?? ""}」吗？此操作会同时清理本地视频文件和封面，不可恢复。`}
        confirmText="确认删除"
        danger
        loading={deleting}
        onCancel={() => {
