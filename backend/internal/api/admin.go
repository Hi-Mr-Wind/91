...
// Spider91ImportEntry is a single video entry in the spider91 import JSON payload.
type Spider91ImportEntry struct {
...
// Spider91ImportResult is the outcome of importing spider91 videos from JSON.
type Spider91ImportResult struct {
...
type AdminServer struct {
	...
	// OnDeleteVideo 删除指定视频的 catalog 记录并清理本地文件（视频、封面、预览）。
	// 返回删除的本地文件数量。视频不存在时返回 sql.ErrNoRows。
	OnDeleteVideo func(ctx context.Context, videoID string) (int, error)
	// OnImportSpider91Videos 将一批 91 视频条目导入到指定的 spider91 drive。
	// entries 已从 JSON 请求体解析并校验。返回导入结果统计。
	OnImportSpider91Videos func(ctx context.Context, driveID string, entries []Spider91ImportEntry) (*Spider91ImportResult, error)
	// Theme 读写（"dark" | "pink"）
...
func (a *AdminServer) Register(r chi.Router) {
	r.Route("/admin/api", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			// spider91 爬虫管理
			r.Post("/spider91/{driveID}/import", a.handleSpider91Import)
			r.Delete("/spider91/{driveID}/videos", a.handleSpider91DeleteVideos)
...
// handleDeleteVideo 删除单个视频（软删除已在 handleHideVideo 中实现，这里是硬删除）。
// 清理本地文件（视频、封面、预览），然后从 catalog 中删除记录。
func (a *AdminServer) handleDeleteVideo(w http.ResponseWriter, r *http.Request) {
	...
	if a.OnDeleteVideo != nil {
		n, err := a.OnDeleteVideo(r.Context(), id)
		if err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				writeErr(w, http.StatusNotFound, err)
			} else {
		...
		// 没有注入清理钩子时，直接删 catalog 记录
		if err := a.Catalog.DeleteVideo(r.Context(), id); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				writeErr(w, http.StatusNotFound, err)
			} else {
				writeErr(w, http.StatusInternalServerError, err)
...
// handleSpider91DeleteVideos 批量删除指定 spider91 drive 下的所有视频。
// 逐个清理本地文件并从 catalog 中删除。
func (a *AdminServer) handleSpider91DeleteVideos(w http.ResponseWriter, r *http.Request) {
	driveID := chi.URLParam(r, "driveID")
	if driveID == "" {
		http.Error(w, "driveID is required", http.StatusBadRequest)
		return
	}

	// 验证 drive 存在且为 spider91 类型
	drive, err := a.Catalog.GetDrive(r.Context(), driveID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			writeErr(w, http.StatusNotFound, err)
		} else {
	...
	if drive.Kind != "spider91" {
		http.Error(w, "drive is not a spider91 drive", http.StatusBadRequest)
		return
	}

	// 列出该 drive 下所有视频
	videos, err := a.Catalog.ListVideosByDriveID(r.Context(), driveID, 0)
	if err != nil {
		writeErr(w, http.StatusInternalServerError, err)
		return
	}
	...
	for _, v := range videos {
		...
		if a.OnDeleteVideo != nil {
			if _, err := a.OnDeleteVideo(r.Context(), v.ID); err != nil {
				if errors.Is(err, sql.ErrNoRows) {
					skipped++
					continue
				}
				log.Printf("[admin] delete spider91 video %s: %v", v.ID, err)
				skipped++
				continue
			}
			deleted++
		} else {
			if err := a.Catalog.DeleteVideo(r.Context(), v.ID); err != nil {
				if errors.Is(err, sql.ErrNoRows) {
					skipped++
					continue
				}
...
// spider91ImportReq is the JSON request body for POST /admin/api/spider91/{driveID}/import.
type spider91ImportReq struct {
	...
	Failed       int                   `json:"failed"`
	Videos       []Spider91ImportEntry `json:"videos"`
}

// handleSpider91Import 从 JSON 导入视频到 spider91 drive。
// 请求体格式与 spider_91porn.py 的 output JSON 一致。
func (a *AdminServer) handleSpider91Import(w http.ResponseWriter, r *http.Request) {
	driveID := chi.URLParam(r, "driveID")
	if driveID == "" {
		http.Error(w, "driveID is required", http.StatusBadRequest)
		return
	...
	//   1. 完整格式：{"videos": [...], ...}
	//   2. 纯视频列表：[{...}, {...}, ...]
	body, err := io.ReadAll(io.LimitReader(r.Body, 16*1024*1024)) // 16 MB max
	if err != nil {
		writeErr(w, http.StatusBadRequest, err)
		return
	}

	var entries []Spider91ImportEntry

	// 先尝试按完整格式解析
	var req spider91ImportReq
	if jsonErr := json.Unmarshal(body, &req); jsonErr == nil && len(req.Videos) > 0 {
		entries = req.Videos
	} else {
		// 再尝试按纯视频列表解析
		var list []Spider91ImportEntry
		if jsonErr := json.Unmarshal(body, &list); jsonErr != nil {
			writeErr(w, http.StatusBadRequest, fmt.Errorf("invalid JSON: expected spider91 import format"))
			return
		}
		entries = list
	}
	...
	if len(entries) == 0 {
		writeErr(w, http.StatusBadRequest, errors.New("no video entries found in request body"))
		return
	}

	// 基本校验
	for i, e := range entries {
		if strings.TrimSpace(e.VideoURL) == "" {
			writeErr(w, http.StatusBadRequest, fmt.Errorf("entry %d: video_url is required", i))
			return
		}
		if strings.TrimSpace(e.SourceID) == "" {
			writeErr(w, http.StatusBadRequest, fmt.Errorf("entry %d (%s): source_id is required", i, e.Title))
			return
		}
	}

	var result *Spider91ImportResult
	if a.OnImportSpider91Videos != nil {
		result, err = a.OnImportSpider91Videos(r.Context(), driveID, entries)
	} else {
		writeErr(w, http.StatusInternalServerError, errors.New("spider91 import is not configured"))
		return
	}
	if err != nil {
		writeErr(w, http.StatusInternalServerError, err)
