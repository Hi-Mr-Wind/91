...
func (s *Server) RegisterRoutes(r chi.Router, a *auth.Authenticator) {
	...
	r.Group(func(r chi.Router) {
		...
		r.Get("/p/ftp/{videoID}", s.handleFtpVideo)
...
// handleFtpVideo serves FTP-sourced videos from the local cache.
func (s *Server) handleFtpVideo(w http.ResponseWriter, r *http.Request) {
	videoID := chi.URLParam(r, "videoID")
	v, err := s.Catalog.GetVideo(r.Context(), videoID)
	if err != nil || v.Hidden {
		http.NotFound(w, r)
	...
	// StreamURL handles on-demand download from FTP server if not cached
	link, err := fd.StreamURL(r.Context(), v.FileID)
	if err != nil {
		log.Printf("[ftp] serve %s: %v", v.ID, err)
		http.Error(w, "ftp download failed", http.StatusServiceUnavailable)
		return
	}
	path := link.URL
...
func (s *Server) videoSource(v *catalog.Video) string {
	...
	if s.Proxy != nil && s.Proxy.Registry != nil {
		...
		if d, ok := s.Proxy.Registry.Get(v.DriveID); ok && d.Kind() == ftp.Kind {
			return "/p/ftp/" + v.ID
		}
	}
