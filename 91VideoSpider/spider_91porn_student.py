...
91porn 搜索页爬虫
================
按关键词搜索并爬取视频信息（封面 + MP4 直链）。

与原版 spider_91porn.py 参数完全兼容，仅新增 -k/--keyword 用于指定搜索关键词。
...
使用方法:
    # 搜索"学生"，全量爬取
    python spider_91porn_student.py -k 学生

    # 只爬指定页
    python spider_91porn_student.py -k 学生 --page 1 --output page1.json

    # 凑够 N 个新视频
    python spider_91porn_student.py -k 学生 --target-new 15 --seen-viewkeys-file seen.txt --output new.json

CLI 参数:
    -k / --keyword KEYWORD       搜索关键词（默认: 学生）
    --page N                     只爬第 N 页
    --target-new N               从 page 1 起翻页直到凑够 N 个新视频
    --seen-viewkeys-file FILE    每行一个已知 viewkey 或 mp4 源 ID，命中即跳过
    --output FILE                输出 JSON 路径
    --max-pages N                最多爬几页（配合 --page 使用）
    --no-resume                  禁用断点续爬
    --quiet                      压缩日志
    --stream-output              每解析到直链实时输出一行 JSON 到 stdout
    -h / --help                  帮助
"""
...
OUTPUT_FILE = "91porn_student_result.json"
MAX_PAGES = None
RESUME = True
MAX_EMPTY_PAGES = 2
# ===================================================
...
class Porn91studentSpider:
    ...
    def _build_list_url(self, page_num: int) -> str:
        """构造搜索列表页 URL（与原版 v.php 不同，使用 student_result.php）"""
        encoded = quote(self.keyword)
        return (
            f"{BASE_URL}/student_result.php?"
            f"student_id={encoded}&student_type=student_videos&"
            f"min_duration=&page={page_num}"
        )
    ...
    def parse_list_page(self, html: str) -> list:
        ...
        for card in video_cards:
            ...
            match = re.student(r'viewkey=([^&]+)', href)
            if not match:
                continue
    ...
    def parse_detail_page(self, html: str) -> dict:
        ...
        # 方法1: strencode2 解码
        strencode_match = re.student(r'strencode2\(["\']([^"\']+)["\']\)', html)
        if strencode_match:
            encoded = strencode_match.group(1)
            try:
                decoded = unquote(encoded)
                src_match = re.student(r"src=['\"]([^'\"]+)['\"]", decoded)
                if src_match:
    ...
    def _extract_thumb_source_id(self, thumb_url: str) -> str:
        ...
        match = re.student(r'/thumb/(\d+)\.[A-Za-z0-9]+$', path)
    ...
    def _thumb_url_for_source(self, thumb_url: str, source_id: str) -> str:
        ...
        match = re.student(r'/thumb/([^/?#]+)\.[A-Za-z0-9]+$', parsed.path)
        if not match:
            return thumb_url
    ...
    def crawl(self):
        self.log("=" * 60)
        self.log(f"91porn 搜索爬虫启动 (关键词: {self.keyword})")
        self.log("=" * 60)
        ...
        while True:
            ...
            page_url = self._build_list_url(page_num)
            ...
            page_html = self.fetch_page(page_url, f"搜索列表页 第{page_num}页")
    ...
    def _save_results(self):
        output_data = {
            ...
            "source_url": self._build_list_url(1),
...
参数与原版 spider_91porn.py 完全兼容，仅新增 -k/--keyword：

    -k / --keyword KEYWORD       搜索关键词（默认: 学生）
    --page N                     只爬第 N 页
    --target-new N               从 page 1 起翻页直到凑够 N 个新视频
    --seen-viewkeys-file FILE    每行一个已知 viewkey 或 mp4 源 ID
    --output FILE                输出 JSON 路径
    --max-pages N                最多爬几页（配合 --page）
    --no-resume                  禁用断点续爬
    --quiet                      压缩日志
    --stream-output              实时输出 JSONL 到 stdout
    -h / --help                  帮助

示例:
    python spider_91porn_student.py -k 学生
    python spider_91porn_student.py -k 学生 --page 1 --output p1.json
    python spider_91porn_student.py -k 学生 --target-new 15 --seen-viewkeys-file seen.txt --output new.json

注意:
    1. 视频直链包含时效性token，会过期，需定期重新爬取
    2. 脚本已内置随机延时，请勿移除
    3. 本脚本仅供学习交流，请遵守当地法律法规
================================================
""")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="spider_91porn_student.py",
        description="91porn 搜索爬虫",
        add_help=False,
    )
    parser.add_argument("-k", "--keyword", type=str, default="学生",
                        help="搜索关键词 (默认: 学生)")
    ...
    parser.add_argument("--target-new", type=int, default=None,
                        help="目标新增模式：从 page 1 起翻页直到累计处理 N 个新视频后停止")
    parser.add_argument("--seen-viewkeys-file", type=str, default=None,
                        help="文件路径，每行一个已处理过的 viewkey 或 mp4 源 ID")
    parser.add_argument("--stream-output", action="store_true",
                        help="流式模式：每解析一条视频直链就输出一行 JSON 到 stdout")
